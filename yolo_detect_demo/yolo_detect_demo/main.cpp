...
void draw_result(cv::Mat& image, std::string label, cv::Rect box) {
	cv::rectangle(image, box, cv::Scalar(255, 0, 0), 3);
	int baseLine;
	...
	cv::putText(image, label, cv::Point(box.x, box.y), cv::FONT_HERSHEY_SIMPLEX, 1.5, cv::Scalar(0, 255, 0), 2);
}
...
int main(int argc, char** argv) {
	...
	//读取测试文件
	std::string top_dir = "\\testImages";
	if (argc > 1) {
		top_dir = std::string(argv[1]);
	}
	std::vector<std::string> fn;
	std::string pattern = std::string(top_dir) + "/*.jpg";

	cv::glob(pattern, fn, false);
	cv::namedWindow("detection", 0);
	for (int i = 0; i < fn.size(); i++) {
		cv::Mat image = cv::imread(fn[i]), blob;
